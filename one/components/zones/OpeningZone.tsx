...
/* ─── Core Intelligence Field ────────────────────────────────────
   Canvas-based ambient atmosphere. Runs entirely in this file.
   No external imports needed.
──────────────────────────────────────────────────────────────── */
function useIntelligenceField(canvasRef: React.RefObject<HTMLCanvasElement>) {
  useEffect(() => {
    ...
    function npos(n: Node, pf: number) {
      const arc = arcs[n.ai], ang = arc.a + n.off, f = pf*(1+n.ai*.60);
      return { x: CX+mx*f+Math.cos(ang)*sc(arc.r0), y: CY+my*f*.70+Math.sin(ang)*sc(arc.r0) };
    }
    ...
    function draw() {
      ...
      cg.addColorStop(0,   `rgba(218,142,50,${.48+breath*.12})`);
      cg.addColorStop(.22, `rgba(178,108,34,${.22+breath*.06})`);
      ...
      pts.forEach(p => {
        ctx!.beginPath(); ctx!.arc(CX+p.x+mx*11,CY+p.y+my*8,p.r,0,Math.PI*2);
        ctx!.fillStyle=p.warm?`rgba(210,178,112,${p.al})`:`rgba(136,164,190,${p.al})`; ctx!.fill();
      });

      const aLen=sc(272)*.72;
      ctx!.beginPath();
      ctx!.moveTo(ox+Math.cos(.265+Math.PI)*aLen*1.35+mx*10, oy+Math.sin(.265+Math.PI)*aLen*1.35+my*7);
      ctx!.lineTo(ox+Math.cos(.265)*aLen*1.15+mx*10, oy+Math.sin(.265)*aLen*1.15+my*7);
      ctx!.strokeStyle='rgba(194,174,128,0.08)'; ctx!.lineWidth=0.55; ctx!.stroke();

      [[0,4],[2,5]].forEach(([ai,bi]) => {
        const a=npos(nodes[ai],4), b=npos(nodes[bi],4);
        ctx!.beginPath(); ctx!.moveTo(a.x,a.y); ctx!.lineTo(b.x,b.y);
        ctx!.strokeStyle='rgba(206,182,136,0.12)'; ctx!.lineWidth=0.50; ctx!.stroke();
      });

      arcs.forEach((arc,i) => {
        const pf=3+i*1.7;
        ctx!.beginPath(); ctx!.arc(CX+mx*pf,CY+my*pf*.70,sc(arc.r0),arc.a,arc.a+arc.sw*Math.PI/180);
        ctx!.strokeStyle=arc.warm?`rgba(155,128,68,${arc.al})`:`rgba(80,114,140,${arc.al})`;
        ctx!.lineWidth=arc.w; ctx!.stroke();
      });

      nodes.forEach((n,i) => {
        const pos=npos(n,4), nr=i<2?3.0:i<4?2.4:1.7, pr=i===0?nr+breath*.65:nr;
        ctx!.beginPath(); ctx!.arc(pos.x,pos.y,pr,0,Math.PI*2);
        ctx!.fillStyle=`rgba(222,194,140,${i<2?.88:i<4?.70:.52})`; ctx!.fill();
        if (i<2) {
          ctx!.beginPath(); ctx!.arc(pos.x,pos.y,pr+4+breath*2.8,0,Math.PI*2);
          ctx!.strokeStyle=`rgba(212,165,78,${.13+breath*.10})`; ctx!.lineWidth=0.60; ctx!.stroke();
        }
      });

      const oa=arcs[3], or_=sc(oa.r0), omx=CX+mx*9, omy=CY+my*6;
      for (let i=0;i<8;i++) {
        const a=oa.a+(i*Math.PI/4), len=i%2===0?7.5:4.0;
        ctx!.beginPath();
        ctx!.moveTo(omx+Math.cos(a)*(or_-len*.5), omy+Math.sin(a)*(or_-len*.5));
        ctx!.lineTo(omx+Math.cos(a)*(or_+len*.5), omy+Math.sin(a)*(or_+len*.5));
        ctx!.strokeStyle=`rgba(200,176,124,${i%2===0?.30:.14})`; ctx!.lineWidth=i%2===0?.75:.50; ctx!.stroke();
      }
    ...
    const onMove = (e: MouseEvent) => {
      const r = hero.getBoundingClientRect();
      tmx=((e.clientX-r.left)/r.width-.5)*2.6; tmy=((e.clientY-r.top)/r.height-.5)*2.0;
    };
    const onLeave = () => { tmx=0; tmy=0; };
    const onTouch = (e: TouchEvent) => {
      e.preventDefault();
      const r=hero.getBoundingClientRect();
      tmx=((e.touches[0].clientX-r.left)/r.width-.5)*1.8; tmy=((e.touches[0].clientY-r.top)/r.height-.5)*1.4;
    };
    ...
    ro.observe(hero);
    resize();
    if (reduced) { t=1.5; draw(); cancelAnimationFrame(raf); }
...
export function OpeningZone() {
  ...
  useIntelligenceField(canvasRef);
  ...
  return (
    ...
    >
      {/* Canvas — animation fills hero, pointer-events off */}
      <canvas
        ref={canvasRef}
        aria-hidden
